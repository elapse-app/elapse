...
class _AuthDelState extends State<AuthDel> {
  ...
  Future<void> updatePass() async {
    ...
    if (emailController1.text == emailController2.text &&
        passwordController0.text == passwordController1.text) {
      final credential = await FirebaseAuth.instance.signInWithEmailAndPassword(
        email: emailController2.text,
        password: passwordController1.text,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Success! Your account has been deleted.'),
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
              'Emails or passwords do not match. Are you sure you typed them correctly?'),
        ),
      );
    }
  }
