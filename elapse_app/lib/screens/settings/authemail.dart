...
class AuthEmail extends StatefulWidget {
  const AuthEmail({super.key, required this.prefs});
...
class _AuthEmailState extends State<AuthEmail> {
  ...
  Future<void> updateEmail() async {
    ...
    if (newEmailController0.text == newEmailController1.text) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('Success! Check your email to verify this change, then sign in again.'),),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('Emails do not match. Are you sure you typed them correctly?'),),
      );
    }
  }
}