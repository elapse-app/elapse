  ...
  Future<void> signUP() async {
    try {
      ...
      if (e.code == 'unknown-error') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('The password provided is too weak. Ensure it is at least 6 characters!')),
        );
        return;
      } else if (e.code == 'email-already-in-use') {
        print('authErr - email-already-in-use');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('An account already exists for that email.'),),
        );
        return;
      }
    } catch (e) {
