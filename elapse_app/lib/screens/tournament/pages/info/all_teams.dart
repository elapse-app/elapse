...
class _AllTeamsState extends State<AllTeams> {
  ...
  void initState() {
    ...
    skillsStats = getWorldSkillsRankings(widget.tournament.seasonID,
    ...
    tmTournament =
        inTM ? loadTournament(prefs.getString("TMSavedTournament")) : null;
  ...
  Widget build(BuildContext context) {
    ...
    if (filter.regions!.isNotEmpty) {
      teams = teams
          .where((e) =>
              filter.regions!.any((e2) => e2 == (e.location?.region ?? "")))
          .toList();
    }
    if (filter.saved) {
      teams = teams
    ...
    if (filter.atTournament && inTM) {
      teams = teams
          .where((e) => tmTournament!.teams.any((e2) => e2.id == e.id))
          .toList();
    }
    if (filter.scouted) {
      teams = teams
    ...
    switch (sortIndex) {
      case 0:
        teams.sort((a, b) {
          num teamNumA =
              num.parse(a.teamNumber!.substring(0, a.teamNumber!.length - 1));
          num teamNumB =
              num.parse(b.teamNumber!.substring(0, b.teamNumber!.length - 1));
          if (teamNumA == teamNumB) {
            return a.teamNumber![a.teamNumber!.length - 1]
                .compareTo(b.teamNumber![b.teamNumber!.length - 1]);
          }
      ...
      case 2:
        teams.sort((a, b) {
          return (loadedVDA!.firstWhereOrNull((e) => e.id == b.id)?.opr ?? -1e5)
              .compareTo(
                  loadedVDA!.firstWhereOrNull((e) => e.id == a.id)?.opr ??
                      -1e5);
        });
        break;
      case 3:
        teams.sort((a, b) {
          return (loadedVDA!.firstWhereOrNull((e) => e.id == a.id)?.dpr ?? 1e5)
              .compareTo(
                  loadedVDA!.firstWhereOrNull((e) => e.id == b.id)?.dpr ?? 1e5);
        });
        break;
      case 4:
        teams.sort((a, b) {
          return (loadedVDA!.firstWhereOrNull((e) => e.id == b.id)?.ccwm ??
                  -1e5)
              .compareTo(
                  loadedVDA!.firstWhereOrNull((e) => e.id == a.id)?.ccwm ??
                      -1e5);
        });
        break;
      case 5:
        teams.sort((a, b) {
          return (loadedVDA!
                      .firstWhereOrNull((e) => e.id == b.id)
                      ?.winPercent ??
                  -1e5)
              .compareTo(loadedVDA!
                      .firstWhereOrNull((e) => e.id == a.id)
                      ?.winPercent ??
                  -1e5);
        });
        break;
      case 6:
        teams.sort((a, b) {
          return (loadedSkills
                      ?.firstWhereOrNull((e) => e.teamId == a.id)
                      ?.rank ??
                  1e5)
              .compareTo(loadedSkills
                      ?.firstWhereOrNull((e) => e.teamId == b.id)
                      ?.rank ??
                  1e5);
        });
        break;
      case 7:
        teams.sort((a, b) {
          return (loadedVDA!.firstWhereOrNull((e) => e.id == b.id)?.trueSkill ??
                  0)
              .compareTo(
                  loadedVDA!.firstWhereOrNull((e) => e.id == a.id)?.trueSkill ??
                      0);
        });
        break;
    }
              ...
              delegate: SliverChildBuilderDelegate(
                (BuildContext context, int index) {
                  ...
                  switch (sortIndex) {
                    ...
                    case 1:
                      subInfo =
                          '${teams[index].location?.city ?? ""}${teams[index].location?.city != null ? "," : ""} ${teams[index].location?.region ?? ""}';
                      break;
                    case 2:
                      subInfo =
                          "OPR ${loadedVDA!.singleWhereOrNull((e) => e.id == teams[index].id)?.opr ?? "—"}";
                      break;
                    case 3:
                      subInfo =
                          "DPR ${loadedVDA!.singleWhereOrNull((e) => e.id == teams[index].id)?.dpr ?? "—"}";
                      break;
                    case 4:
                      subInfo =
                          "CCWM ${loadedVDA!.singleWhereOrNull((e) => e.id == teams[index].id)?.ccwm ?? "—"}";
                      break;
                    case 5:
                      subInfo =
                          "${loadedVDA!.singleWhereOrNull((e) => e.id == teams[index].id)?.winPercent ?? "—"}%";
                      break;
                    case 6:
                      subInfo =
                          "${loadedSkills?.singleWhereOrNull((e) => e.teamId == teams[index].id)?.score ?? "—"} pts";
                      break;
                    case 7:
                      subInfo = loadedVDA!
                              .singleWhereOrNull((e) => e.id == teams[index].id)
                              ?.trueSkill
                              .toString() ??
                          "—";
                      break;
